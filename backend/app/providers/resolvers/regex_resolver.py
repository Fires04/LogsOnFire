"""Safe regex-based log discovery: walk a base directory (locally or via SFTP)
and keep files whose path relative to that base directory matches a regex.

The regex is only ever evaluated in Python against filenames already listed
through a safe directory-listing call (os.scandir locally, SFTP readdir
remotely) — it never touches a shell, so it cannot be used to execute
anything, regardless of what the base_dir or pattern strings contain.
"""
...
from app.providers.base import MAX_RESOLVED_FILES, MAX_WALK_ENTRIES, ResolvedFile
